...
class IndexAction extends Action {	
    ...
    public function index(){ 	
		...
		if(session('?username')){
			...
			$news_list=$news->field(array('id','subject','author','createtime','lastmodifytime','sum'))->order('id desc')->limit($page->firstRow.','.$page->listRows)->select();
  ...
  /**
    ...
    function delete(){
    	...
    	//跳转至Article控制器来实现
    	if($_GET['id']){
    		redirect(U('/News/delete/id/'.$_GET['id']),0, '删除文章');
    	}
    	else{
    		$this->error('参数错误！');
    	}
    ...
    function edit(){
    	if($_GET['id']){
    		redirect(U('/News/edit/id/'.$_GET['id']),0, '删除文章');
    	}
    	else{
    		$this->error('参数错误！');
    	}
